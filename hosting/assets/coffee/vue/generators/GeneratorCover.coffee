Vue.component 'GeneratorCover', (resolve, reject) ->
  Vue.http.get('/template/generators/cover.html').then (template) ->
    resolve
      ...
      beforeMount: ->
        ...
        @frame_image_select = q_gen_cover(@config.frame_images[0])
        @base64ify 'basemap_image'

      mounted: ->
        if @is_desktop
          new ResizeSensor($('#frame_container'), @window_resize)
          new ResizeSensor($('#generator_container'), @window_resize)
