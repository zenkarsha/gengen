var PoemGen = function() {
  ...
  function load_dict_gram(cb) {
    if (isEmptyObject(_dict) || isEmptyObject(_gram2) || isEmptyObject(_yun) || _gram1.length === 0) {
      var dic = dict_json;
      var gram1 = gram1_json;
      var gram2 = gram2_json;
      for (let key in dic) {
        _dict[key] = dic[key];
  ...
  function viterbi_sub_2(pre_ary, this_ary, backward) {
    ...
    for (let tw in this_ary) {
      var max_prob = 0
      var rand_pw = null;
      var all_temp_prob = []
      for (let pw in pre_ary) {
  ...
  function viterbi_sub_1(word_start, interval, backward) {
    var pre_ary = {};
    pre_ary[word_start.slice(-1)[0]] = {
      'prob': 1.0,
      'word': word_start.slice(0, word_start.length - 1)
    };
    for (let i = 0; i < interval; i++) {
      var word_list = select_words();
      var this_ary = {};
      for (let j in word_list) {
        this_ary[word_list[j]] = {
          'prob': 0.0,
          'word': []
        };
      }
  ...
  function gen_poem_yun(word_yun, itval_dict) {
    ...
    for (let y in _yun) {
      yun_ary[y] = [];
      for (let i = 0; i < word_yun.length; i++) {
        yun_ary[y].push({
          'prob': 0,
          'word': []
        });
    ...
    for (let i = 0; i < word_yun.length; i++) {
      ...
      for (let w in pre_ary) {
        var yun = _dict[w]['yun'];
        var last_word_ary = {};
        for (let y in yun_ary) {
          for (let j = 0; j < i + 1; j++) {
    ...
    var max_yun = null;
    for (let yun in yun_ary) {
      var yun_prob_product = 1;
      for (let i in yun_ary[yun]) {
  ...
  function gen_poem() {
    ...
    for (let i = 0; i < _raw_str.length; i++) {
      if (!_itval_slash) {
